...
.ready(
		function() {
			...
			var rules = {
		        ...
		        $('#formMascotaEncontrada_validation').validate({
		       	    rules: rules,
		       	    messages: messages,
		       	    submitHandler: function (form) {
		       	    	
		       	    		       	    		
		       	    	var formData = new FormData($("#formMascotaEncontrada_validation")[0]);
			 ...
			 var modelJson =$('#dataEncontradaMascota').val();
	            ...
	            if(modelJson!=null && modelJson!=''){
	            	...
	            	$('#id').val(data.id);
	            	$("#pdfMascotaEncontrada").prop("href", "/pdp/descargarPosterpdfEncontrada?id="+data.id);
	            	...
	            	$('#fechaEncontrado').val(getFecha(data.fechaEncontrado));
	          		$('#descripcion').val(data.mascota.caracteristica);
	          		$('#comentarioAdicional').val(data.descripcion);
	          		...
	          		$("#tipoEncuentro option[value='" + data.tipoEncuentro
										+ "']")
					.attr("selected", "selected");
	            	...
	            	if(data.encontradoMascotaFotos!=null && data.encontradoMascotaFotos.length>0){
	            		for (var i = 0; i < data.encontradoMascotaFotos.length; i+=1) {

		          				$('#verFoto').show();
			          			$('#fotoMascota').attr('src','/pdp/'+data.encontradoMascotaFotos[i].foto);
			          			indPrincipal++;
	            ...
	            $('#fechaEncontrado').inputmask('dd/mm/yyyy', { placeholder: '__/__/____' });
